<template>
  <el-row>
    <el-row class="tool-bar">
      ...
      <el-button type="primary" size="mini" icon="el-icon-delete" @click="deleteDoctor">删除</el-button>
    ...
    <el-row class="table">
      ...
      >
        ...
        <el-table-column sortable prop="qualifications" label="学历" align="center" />
        <el-table-column sortable prop="jobTitle" label="职称" align="center" />
    ...
    <el-row class="form">
      <el-dialog title="添加医生信息" :visible.sync="dialogFormVisible" :close-on-click-modal="false" :show-close="false">
        <el-form :model="form">
          <el-row>
            <el-col :span="10">
...
<script>
import { doctorList, deleteDoctor, addDoctor, updateDoctor, downloadExcelTemplate, exportDataToExcle } from '@/api/doctor'
import { formatDate } from '@/utils/formatter'
import { getDepartment } from '@/api/department'
import { validDoctorForm } from '@/utils/valid'
import Vue from 'vue'
  ...
  methods: {
    ...
    edit() { // 修改病人信息
      ...
      if (selectedData.length > 1) {
        ...
        this.form.qualifications = selectedData[0].qualifications
        this.form.jobTitle = selectedData[0].jobTitle
    ...
    deleteDoctor() {
      ...
      this.$confirm(names.toString(), '确定删除下列医生吗？', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteDoctor(array).then(response => {
    ...
    submitForm() { // 提交表单中的数据
      if (validDoctorForm(this.form)) { // 先验证表单中的数据是否正确
    ...
    uploadUrl() {
      return `${process.env.VUE_APP_BASE_API}/doctor/uploadExcel` // 需要区分环境所以用process.env.VUE_APP_BASE_API获取基本域名/ip
    },
